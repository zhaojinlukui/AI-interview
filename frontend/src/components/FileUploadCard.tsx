...
export interface FileUploadCardProps {
  ...
  /** 多文件上传回调 */
  onUploadMultiple?: (files: File[], names?: string[]) => void;
  /** 返回回调 */
  onBack?: () => void;
}
...
export default function FileUploadCard({
  ...
  const updateSelectedFiles = useCallback((fileList: FileList | File[]) => {
  ...
  }, [maxFiles, multiple, onFileSelect, onFilesSelect]);
  ...
  const handleUpload = () => {
    ...
    if (multiple && onUploadMultiple) {
      const names = selectedFiles.length === 1 ? [name.trim()] : [];
      onUploadMultiple(selectedFiles, names);
      return;
    }
    onUpload?.(selectedFiles[0], name.trim() || undefined);
  };
  ...
  return (
    ...
    >
                ...
                {multiple ? (
                  <div className="max-w-xl mx-auto space-y-3">
                    <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
                      已选择 {selectedFiles.length} 个文件，最多 {maxFiles} 个
                    </p>
                    <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                      {selectedFiles.map((file) => (
                        <div
                          key={`${file.name}-${file.lastModified}-${file.size}`}
                          className="flex items-center justify-center gap-4 bg-slate-50 dark:bg-slate-700/50 px-6 py-4 rounded-xl"
                        >
                  ...
                  <div
                    ...
                    <div className="text-left flex-1 min-w-0">
                      <p className="font-semibold text-slate-900 dark:text-white truncate">{selectedFiles[0].name}</p>
                      <p className="text-sm text-slate-500 dark:text-slate-400">{formatFileSize(selectedFiles[0].size)}</p>
                    </div>
                    <button
                      className="w-8 h-8 bg-red-100 dark:bg-red-900/50 text-red-500 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/70 transition-colors flex items-center justify-center"
                      onClick={(e) => {
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-slate-400 dark:text-slate-500 mb-4">
                    {formatHint}（{maxSizeHint}{multiple ? `，单次最多 ${maxFiles} 个` : ''}）
                  </p>
                </div>
                <motion.button
      ...
      {/* 名称输入框 */}
      {showNameInput && selectedFiles.length === 1 && (
        <motion.div
        ...
        {selectedFiles.length > 0 && (
          ...
          >
            {uploading ? (
              ...
              selectedFiles.length > 1 ? `${uploadButtonText}（${selectedFiles.length} 个）` : uploadButtonText
            )}
          </motion.button>
        )}
      </div>

...
interface AnalysisPanelProps {
  ...
  onReanalyze?: () => void;
  reanalyzing?: boolean;
  scoreMaxima?: ResumeScoreMaxima;
}

interface ResumeScoreMaxima {
...
const DEFAULT_SCORE_MAXIMA: ResumeScoreMaxima = {
  projectWeight: 40,
  skillMatchWeight: 20,
  contentWeight: 15,
  structureWeight: 15,
...
export default function AnalysisPanel({
  ...
  onReanalyze,
  reanalyzing,
  scoreMaxima = DEFAULT_SCORE_MAXIMA,
}: AnalysisPanelProps) {
  // 准备雷达图数据
  const radarData = useMemo(() => {
    ...
    const projectFullMark = scoreMaxima.projectWeight;
    const skillMatchFullMark = scoreMaxima.skillMatchWeight;
    const contentFullMark = scoreMaxima.contentWeight;
    const structureFullMark = scoreMaxima.structureWeight;
    const expressionFullMark = scoreMaxima.expressionWeight;
        ...
        >
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <ScoreProgressBar
              label="项目经验"
              score={projectScore}
              maxScore={scoreMaxima.projectWeight}
              color="bg-purple-500"
              delay={0.3}
            ...
            <ScoreProgressBar
              label="技能匹配"
              score={skillMatchScore}
              maxScore={scoreMaxima.skillMatchWeight}
              color="bg-blue-500"
              delay={0.4}
            />
            <ScoreProgressBar
              label="内容完整性"
              score={contentScore}
              maxScore={scoreMaxima.contentWeight}
            ...
            <ScoreProgressBar
              label="结构清晰度"
              score={structureScore}
              maxScore={scoreMaxima.structureWeight}
              color="bg-cyan-500"
              delay={0.6}
            />
            <ScoreProgressBar
              label="表达专业性"
              score={expressionScore}
              maxScore={scoreMaxima.expressionWeight}
              color="bg-orange-500"
              delay={0.7}
            />
          </div>

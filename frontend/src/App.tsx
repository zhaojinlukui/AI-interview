...
const MistakePage = lazy(() => import('./pages/MistakePage'));
const LoginPage = lazy(() => import('./pages/LoginPage'));
const AdminUsersPage = lazy(() => import('./pages/AdminUsersPage'));
const AdminAiParametersPage = lazy(() => import('./pages/AdminAiParametersPage'));
...
interface LayoutOutletContext {
  openInterviewModalWithResume: (resumeId: number) => void;
}
...
function ResumeDetailWrapper() {
  ...
  const navigate = useNavigate();
  const { openInterviewModalWithResume } = useOutletContext<LayoutOutletContext>();
        ...
        <Suspense fallback={<Loading />}>
          <Routes>
            ...
            <Route path="/" element={<RequireAuth><Layout /></RequireAuth>}>
              ...
              <Route path="admin/ai-parameters" element={<RequireAdmin><AdminAiParametersPage /></RequireAdmin>} />
...
function DefaultHomeRedirect() {
  ...
  return <Navigate to={isAdmin ? '/admin' : '/history'} replace />;
}
...
function AdminRouteGate() {
  ...
  const allowedPaths = ['/profile', '/admin', '/admin/users', '/admin/ai-parameters'];
...
function KnowledgeBaseUploadPageWrapper() {
  ...
  const handleUploadComplete = (_result: UploadKnowledgeBaseBatchResponse) => {
    // 上传完成后返回管理页面
    navigate('/knowledgebase');
  };

...
function isEvaluateCompleted(item: UnifiedInterviewItem): boolean {
  if (item.overallScore !== null) return true;
...
function isEvaluating(item: UnifiedInterviewItem): boolean {
  if (item.overallScore !== null) return false;
  return item.evaluateStatus === 'PENDING' || item.evaluateStatus === 'PROCESSING';
}

function getEvaluatingTypes(items: UnifiedInterviewItem[]): Set<UnifiedInterviewItem['type']> {
  return new Set(items.filter(i => isEvaluating(i)).map(i => i.type));
}
...
export default function InterviewHistoryPage({ onBack: _onBack, onViewInterview, onRestartInterview, onContinueInterview }: InterviewHistoryPageProps) {
  ...
  const itemsRef = useRef<UnifiedInterviewItem[]>([]);
  ...
  const loadAll = useCallback(async (isPolling = false) => {
    ...
    try {
      ...
      const currentItems = isPolling ? itemsRef.current : [];
      const evaluatingTypes = isPolling ? getEvaluatingTypes(currentItems) : null;
      const shouldLoadText = !isPolling || evaluatingTypes?.has('text');
      const shouldLoadVoice = !isPolling || evaluatingTypes?.has('voice');

      const [textInterviews, voiceSessions] = await Promise.all([
        shouldLoadText
          ? loadTextInterviews(loadedSkills)
          : Promise.resolve(currentItems.filter(i => i.type === 'text')),
        shouldLoadVoice
          ? loadVoiceInterviews()
          : Promise.resolve(currentItems.filter(i => i.type === 'voice')),
      ]);
  ...
  useEffect(() => {
    ...
    if (hasEvaluating && !pollingRef.current) {
      pollingRef.current = window.setInterval(() => loadAll(true), ASYNC_TASK_POLL_INTERVAL_MS);

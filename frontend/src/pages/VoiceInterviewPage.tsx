...
export default function VoiceInterviewPage() {
  ...
  const handlePhaseConfig = useCallback(async (config: {
    ...
    try {
      ...
      const wsUrl = session.webSocketUrl || `${window.location.protocol === 'https:' ? 'wss' : 'ws'}://${window.location.host}/ws/voice-interview/${session.sessionId}`;

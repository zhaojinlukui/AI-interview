...
- **单实例模型设置**：设置页统一维护 OpenAI 兼容 Base URL、API Key、聊天模型与向量模型，不再区分多 Provider。
- **语音服务配置**：支持在设置页维护 Qwen/DashScope ASR / TTS 参数并执行连通性测试，语音 Key 默认继承主模型且可单独覆盖。
...
│   └── vite.config.ts
│
├── docker-compose.yml                # 完整部署：前端 + 后端 + PostgreSQL + Redis + RustFS
...
```bash
# 创建并编辑 .env
APP_AI_MODEL_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
APP_AI_MODEL_API_KEY=your_model_api_key
APP_AI_MODEL_CHAT_MODEL=qwen3.5-flash
APP_AI_MODEL_EMBEDDING_MODEL=text-embedding-v3
APP_AI_MODEL_EMBEDDING_DIMENSIONS=1024
```
...
```bash
# macOS / Linux（zsh）
echo 'export APP_AI_MODEL_API_KEY=your_api_key' >> ~/.zshrc
source ~/.zshrc

# Linux（bash）
echo 'export APP_AI_MODEL_API_KEY=your_api_key' >> ~/.bashrc
source ~/.bashrc
```
...
创建 `.env` 时，PostgreSQL、Redis、RustFS 推荐与 `docker-compose.yml` 保持一致（数据库用户 `postgres` / 密码 `password`，RustFS `rustfsadmin` / `rustfsadmin`）。主要必填项是 `APP_AI_MODEL_API_KEY`；若你曾在旧版本中使用过不同的库密码或对象存储密钥，请同步修改 `.env`，必要时重建 Postgres 卷以免旧数据与密码不一致。
...
# 可选：APP_AI_MODEL_CHAT_MODEL=qwen3.5-flash
# 也可以在设置页维护通用模型与语音 ASR / TTS 配置
#
# 面试参数配置（可选）：
...
> **仅启动依赖服务**：如果只想本地开发调试（用 `./gradlew :app:bootRun` 或 IDEA 启动后端），可以只启动基础设施：`docker compose up -d postgres redis rustfs`。在项目根目录创建 `.env` 并填写 `APP_AI_MODEL_API_KEY` 即可，基础设施默认账号与 `docker-compose.yml` 一致；平台管理员账号默认不自动创建。
...
检查一下设置页中的 OpenAI 兼容 Base URL、API Key 和聊天模型是否配置正确；如果使用 DashScope，可在阿里云百炼控制台申请 Key（<https://bailian.console.aliyun.com/>）。

### Q: 设置页新增/切换模型后不生效？

用户 AI 配置保存在数据库 `user_ai_settings` 表中。可以在设置页直接保存通用模型与 ASR / TTS 配置；Docker 部署时只需要持久化 PostgreSQL 数据卷。旧 `llm-providers.*` 文件、运行时 AI 设置文件和旧 Provider 数据库表/列不再读取。

### Q: 语音面试无法识别或没有声音？

语音面试的 ASR/TTS 默认继承 `APP_AI_MODEL_API_KEY`，也可以在设置页单独覆盖 ASR/TTS Key。请检查浏览器麦克风权限、后端日志中的 DashScope WebSocket 连接状态，以及设置页里的 ASR/TTS 测试结果。无耳机时可能触发回声录入，建议先使用手动提交模式或佩戴耳机测试。

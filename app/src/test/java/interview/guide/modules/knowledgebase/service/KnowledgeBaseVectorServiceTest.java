 ...
 * <p>测试覆盖：
 * <ul>
 *   <li>向量化存储（vectorizeAndStore）- 分批处理逻辑、metadata 设置、失败清理</li>
...
class KnowledgeBaseVectorServiceTest {
    ...
    class VectorizeAndStoreTests {
        ...
        void testVectorizeSmallContent() {
            ...
            // When: 执行向量化
            int chunkCount = vectorService.vectorizeAndStore(knowledgeBaseId, content);

            // Then: 验证成功向量化时不预删旧数据
            assertTrue(chunkCount > 0, "向量化应返回实际生成的 chunk 数");
        ...
        @Test
        @DisplayName("向量化成功时不预删旧数据")
        void testDoesNotDeleteOldDataBeforeVectorize() {
            // Given: 使用足够长的内容确保产生 chunks
            Long knowledgeBaseId = 1L;
            String content = generateLongContent(10);
            ...
            // Then: 成功路径只添加向量，不删除旧数据
            verify(vectorStore, atLeastOnce()).add(anyList());
            verify(vectorRepository, never()).deleteByKnowledgeBaseId(knowledgeBaseId);
        }

        @Test
        @DisplayName("向量化写入失败时清理 partial chunks 并抛出异常")
        void testVectorizeFailureCleansPartialChunksAndThrowsException() {
            // Given: 使用足够长的内容确保产生 chunks
            Long knowledgeBaseId = 1L;
            String content = generateLongContent(10);
        ...
        void testVectorizeEmptyContent() {
            ...
            // Then: 空内容不会产生 chunks，也不会触发预删或失败清理
            assertEquals(0, chunkCount, "空内容应返回 0 个 chunk");
            verify(vectorRepository, never()).deleteByKnowledgeBaseId(knowledgeBaseId);

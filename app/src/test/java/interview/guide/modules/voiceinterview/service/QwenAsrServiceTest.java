...
class QwenAsrServiceTest {
    ...
    @BeforeEach
    void setUp() {
        settingsResolver = mock(AiSettingsResolver.class);
        ...
        asrService = new QwenAsrService(properties, new AiProperties(), settingsResolver);
    }
    ...
    @Test
    @DisplayName("Should inherit API key from main model when ASR override is empty")
    void testInheritApiKeyFromMainModel() throws Exception {
        VoiceInterviewProperties properties = new VoiceInterviewProperties();
        properties.getQwen().getAsr().setApiKey(null);
        AiProperties aiProperties = new AiProperties();
        aiProperties.getModel().setApiKey("main-model-key");

        QwenAsrService service = new QwenAsrService(properties, aiProperties, settingsResolver);

        assertEquals("main-model-key", field(service, "apiKey"));
    }

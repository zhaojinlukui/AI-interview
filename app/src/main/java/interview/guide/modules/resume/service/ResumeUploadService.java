...
public class ResumeUploadService {
    ...
    public Map<String, Object> uploadAndAnalyze(MultipartFile file) {
        ...
        log.info("收到简历上传请求: {}, 大小: {} bytes ({}), 上传开始处理", fileName, fileSize, formatFileSize(fileSize));
        ...
        if (existingResume.isPresent()) {
            log.info("简历校验处理完成（重复）: {} - 耗时: {}ms", fileName, System.currentTimeMillis() - startTime);
        ...
        log.info("简历已存储: {} - 存储耗时: {}ms", fileKey, System.currentTimeMillis() - storageStart);

        // 5. 发送分析任务到 RabbitMQ（异步处理）
        analyzeStreamProducer.sendAnalyzeTask(savedResume.getId(), resumeText);
        long totalTime = System.currentTimeMillis() - startTime;
        log.info("简历上传处理完成: {}, resumeId={} - 总耗时: {}ms (校验+解析+存储+入库)",

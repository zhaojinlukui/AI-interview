...
public class InterviewPersistenceService {
    ...
    public InterviewSessionEntity saveSession(String sessionId, Long resumeId,
                                              ...
                                              String skillId,
                                              String difficulty) {
        return saveSession(sessionId, resumeId, totalQuestions, questions, skillId, difficulty,
            InterviewSessionEntity.PracticeType.NORMAL, null, null);
    }

    /**
     * 保存新的面试会话，并记录可选的错题重练来源。
     */
    @Transactional(rollbackFor = Exception.class)
    public InterviewSessionEntity saveSession(String sessionId, Long resumeId,
                                              int totalQuestions,
        ...
        try {
            ...
            session.setPracticeType(practiceType != null
                ? practiceType
                : InterviewSessionEntity.PracticeType.NORMAL);
            session.setSourceType(sourceType);
            session.setSourceSessionId(sourceSessionId);
    ...
    public InterviewAnswerEntity saveAnswer(String sessionId, int questionIndex,
        ...
        answer.setCategory(InterviewQuestionDTO.normalizeCategory(category));
    ...
    public void saveReport(String sessionId, InterviewReportDTO report) {
        try {
            ...
            for (InterviewReportDTO.QuestionEvaluation eval : report.questionDetails()) {
                ...
                String normalizedCategory = InterviewQuestionDTO.normalizeCategory(eval.category());
                ...
                if (answer == null) {
                    ...
                    answer.setCategory(normalizedCategory);
                    answer.setUserAnswer(null);  // 未回答
                    log.debug("为未回答的题目 {} 创建答案记录", eval.questionIndex());
                } else if (answer.getCategory() == null || answer.getCategory().isBlank()) {
                    answer.setCategory(normalizedCategory);
                } else {
                    answer.setCategory(InterviewQuestionDTO.normalizeCategory(answer.getCategory()));
                }
    ...
    public List<InterviewSessionEntity> findAll() {
        return sessionRepository.findAllByUserIdOrderByCreatedAtDesc(
            CurrentUserContext.getRequiredUserId()
        );
    }
    ...
    public void deleteSessionsByResumeId(Long resumeId) {
        List<InterviewSessionEntity> sessions = sessionRepository.findByResumeIdAndUserIdOrderByCreatedAtDesc(
            resumeId,
            CurrentUserContext.getRequiredUserId()
        );
        if (!sessions.isEmpty()) {
    ...
    public void deleteSessionBySessionId(String sessionId) {
        Optional<InterviewSessionEntity> sessionOpt = sessionRepository.findBySessionIdAndUserId(
            sessionId,
            CurrentUserContext.getRequiredUserId()
        );
        if (sessionOpt.isPresent()) {
    ...
    public List<HistoricalQuestion> getHistoricalQuestions(String skillId, Long resumeId) {
        ...
        List<HistoricalQuestion> result = sessions.stream()
            .map(InterviewSessionEntity::getQuestionsJson)  // 提取问题列表
            .filter(json -> json != null && !json.isEmpty())  // 过滤空 Json 字符串
            .flatMap(json -> {
                try {
                    // 将Json反序列化为 InterviewQuestionDTO 列表
                    List<InterviewQuestionDTO> questions = objectMapper.readValue(json, new TypeReference<>() {});
                    // 仅保留主问题，抛出追问
            ...
            .filter(hq -> seen.add(hq.question())) // 利用LinkedHashSet.add方法
            .limit(MAX_HISTORICAL_QUESTIONS) // 限制返回数量
            .toList();
